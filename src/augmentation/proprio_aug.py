...
class DriftSimulation:
    ...
    def __call__(self, proprio: torch.Tensor) -> torch.Tensor:
        ...
        if self.drift_type == 'linear':
            # Linear drift along time, per joint (shape [T] × [D] → [T, D])
            t_ramp = torch.linspace(0, 1, T, device=proprio.device).unsqueeze(-1)
            joint_dir = (2 * torch.rand(1, D, device=proprio.device) - 1) * self.drift_magnitude
            drift = t_ramp * joint_dir
            if batch:
